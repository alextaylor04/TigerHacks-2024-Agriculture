...
def crop_description(pre1, pre2, pre3, lat, lon, temp, humidity, ph, rainfall, N, P, K):
    resp = client.chat.completions.create(
        model = "gpt-4-turbo",
        messages = [
            {"role": "user", "content": provide_description(pre1, pre2, pre3, lat, lon, temp, humidity, ph, rainfall, N, P, K)}],
...
def provide_description(pre1, pre2, pre3, lat, lon, temp, humidity, ph, rainfall, N, P, K): # 1 paragraph 3 sentences
    prompt = (
        f"According to our calculations, we have decided we predicted that the best crops to plant are these: {pre1} {pre2} {pre3}\n"
        f"Here is some important information about Latitude: {lat}, Longitude: {lon}:\n"
        f"temperature - temperature in degrees Celsius: {temp}\n"
        f"humidity - relative humidity in %: {humidity}\n"
        f"ph - pH value of the soil: {ph}\n"
        ...
        f"P - ratio of Phosphorous content in soil: {P}\n"
        f"K - ratio of Potassium content in soil: {K}\n"
        "Why did we choose each of these predicted crops?\n"
...
answer1part1, answer1part2, answer1part3, answer2part1, answer2part2, answer2part3, answer3part1, answer3part2, answer3part3 = crop_description(pre1, pre2, pre3, lat, lon, temp_, humidity_, ph_, precipitation_, nitrogen_, phosporus_, potassium_)
# print(answer1part1)
# print(answer1part2)
# print(desc3)
 # non_american_plant, lat, lon, temp, humidity, ph, rainfall, N, P, K
# provide_description(pre1, pre2, pre3, lat, lon, temp_, humidity_, ph_, precipitation_, nitrogen_, phosporus_, potassium_)
